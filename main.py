...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="📞 Contact", url="https://t.me/CHOSEN_ONEx_bot")]])
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="🌟 Support Group ", url="https://t.me/GeniusJunctionX"),
            InlineKeyboardButton(text="🌟 Main Channel ", url="https://t.me/II_LevelUP_II"),
        ],
    ]
)
...
image_urls = [
    "https://postimg.cc/KK11Bcg0.jpg",
    " https://postimg.cc/SnN2QfQFjpg",
...
async def txt_handler(client: Client, m: Message):
    await bot.send_message(m.chat.id, text= (
        ...
        f"╭────────⊰◆⊱────────╮\n"   
        f" ➠ 𝐌𝐚𝐝𝐞 𝐁𝐲: [𝗖𝗛𝗢𝗦𝗘𝗡 𝗢𝗡𝗘 ⚝](http://t.me/CHOSEN_ONEx_bot)\n"
        f"╰────────⊰◆⊱────────╯\n"
        )
    )                    
...
async def txt_handler(bot: Client, m: Message):
    ...
    if m.chat.id != OWNER:
        ...
        await bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💎 Contact Owner", url="http://t.me/CHOSEN_ONEx_bot")]
                ]
            )
        )
        return

    # ✅ OWNER's authorized flow
    editable = await m.reply_text(
        f"**__Hii, I am non-drm Downloader Bot__**\n\n"
        f"<blockquote><i>📎 Send me your TXT file with format:\n"
        f"<code>Name: https://link.com/video</code>\n</i></blockquote>\n"
        f"<blockquote><i>⏱ Auto input ends in 20 seconds...</i></blockquote>"
    )
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:
                cc = f'🌟────•~❉{str(count).zfill(3)}❉~•────🌟\n\n☘️𝗧𝗶𝘁𝗹𝗲 ➤ `{name1} [{res}p] CHOSEN ONE ⚝.mkv\n\n<pre><code>💠 𝗕𝗮𝘁𝗰𝗵 ➤ {b_name}</blockquote>\n\n>𖣐 𝗫𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆: {CR}\n'
                cc1 = f'🌟────•~❉{str(count).zfill(3)}❉~•────🌟\n\n☘️𝗧𝗶𝘁𝗹𝗲 ➤ `{name1} .pdf`\n<blockquote>💠 𝗕𝗮𝘁𝗰𝗵 ➤ {b_name}</blockquote>\n\n>𖣐 𝗫𝘁𝗿𝗮𝗰𝘁𝗲𝗱 𝗕𝘆: {CR}\n'
